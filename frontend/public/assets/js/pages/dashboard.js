  ...
  try {
    ...
    if (projList) {
      if (projects.length === 0) {
        projList.innerHTML = '<tr><td colspan="5" class="text-center py-4 text-muted">Nenhum projecto ainda. <a href="/pages/projects.html">Criar projecto</a></td></tr>';
      } else {
        projList.innerHTML = projects.slice(0,5).map(p => `
          <tr>
            <td><a href="/pages/project-detail.html?id=${p.id}" class="text-decoration-none text-white">${p.title}</a></td>
            <td><span class="badge bg-secondary">${p.type}</span></td>
            <td><span class="status status-${p.status}">${p.status}</span></td>
            <td>${new Date(p.created_at).toLocaleDateString('pt-PT')}</td>
            <td><a href="/pages/project-detail.html?id=${p.id}" class="btn btn-sm btn-outline-secondary">Ver</a></td>
          </tr>
        `).join('');
      }
    }
    ...
    if (stList) {
      stList.innerHTML = startups.length === 0
        ? '<p class="text-muted small">Nenhuma startup criada. <a href="/pages/startups.html">Criar startup</a></p>'
        : startups.slice(0,4).map(s => `
          <a href="/pages/startup-detail.html?id=${s.id}" class="d-block p-3 rounded-3 mb-2 text-decoration-none"
